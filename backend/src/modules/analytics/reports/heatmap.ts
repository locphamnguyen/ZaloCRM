...
export async function getResponseHeatmap(
  ...
  // zaloAccountId → filter on conversations.zalo_account_id.
  // assignedUserId → join conversations.contact_id → contacts.assigned_user_id.
  const rows = await (
    zaloAccountId && assignedUserId
      ? prisma.$queryRaw<Array<{ dow: number; hour: number; avg_seconds: number; sample_count: bigint }>>`
          WITH ranked AS (
            SELECT m.id, m.conversation_id, m.sender_type, m.sent_at,
            ...
            WHERE c.org_id = ${orgId}
              ...
              AND ct.assigned_user_id = ${assignedUserId}
              AND m.sent_at >= ${from}::date
              AND m.sent_at <  (${to}::date + INTERVAL '1 day')
              AND m.is_deleted = false
          ),
          pairs AS (
            SELECT EXTRACT(DOW FROM sent_at)::int AS dow, EXTRACT(HOUR FROM sent_at)::int AS hour,
              EXTRACT(EPOCH FROM (next_sent_at - sent_at))::float AS response_seconds
            FROM ranked
            WHERE sender_type = 'contact' AND next_sender_type = 'self'
              AND next_sent_at - sent_at <= INTERVAL '24 hours'
              AND next_sent_at - sent_at > INTERVAL '0 seconds'
          )
          SELECT dow, hour, AVG(response_seconds)::float AS avg_seconds, COUNT(*) AS sample_count
          FROM pairs GROUP BY dow, hour ORDER BY dow, hour
        `
      : zaloAccountId
        ? prisma.$queryRaw<Array<{ dow: number; hour: number; avg_seconds: number; sample_count: bigint }>>`
            WITH ranked AS (
